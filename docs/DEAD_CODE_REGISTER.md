...
|---|---|---|---|
| Cyclic Count management | `cyclic-count.html` + `cyclic-count.js` (~2,500 lines), `count-form.html`, `count-form.js` | `count_sessions` = 1 row | **Broken anyway** — requires `cin7-sync-service.js` and `sync-utils.js`, neither of which exists. Superseded by the Re-Stock V2 count sheets. Export the single session, then delete the pages and the `/api/audit/*`, `/api/cyclic-sync*`, `/api/sync-audit-data`, `/api/fetch-product` routes in `server.js:661-952` |
